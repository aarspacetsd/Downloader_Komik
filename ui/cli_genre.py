...
def select_chapters(total_chapters: int) -> list[int]:
    ...
    while True:
        ...
        try:
            ...
            choice = inputimeout(prompt="Select an option: ", timeout=10)
...
def run_cli():
    ...
    while True:
        ...
        console.print("2. Search manga by genre")  # Menambahkan opsi genre
        console.print("3. Download by URL")
        ...
        if choice == 1:
                ...
                current_results = Doujindesu.search_by_genre(genre)
                ...
                console.print(f"[cyan]Found {len(current_results.results)} manga[/cyan]\n")
                ...
                if selected_option == "Previous page":
                    ...
                    # Fitur untuk mendownload semua manga
                    for manga in current_results.results:
                        try:
                            manga_obj = Doujindesu(manga.url)
                            details = manga_obj.get_details()
                            console.print(f"\n[cyan]Downloading manga: {details.name}[/cyan]")

                            chapters = manga_obj.get_all_chapters()
                            if not chapters:
                            ...
                            for idx in selected_indices:
                                ...
                                manga_obj.url = chapter_url
                                images = manga_obj.get_all_images()
                            ...
                            console.print(f"[red]Error downloading manga {manga.name}: {e!s}[/red]")
