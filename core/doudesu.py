...
class Doujindesu(ImageToPDFConverter):
    ...
    @classmethod
    def search_by_genre(cls, genre: str, page: int | None = None) -> SearchResult | None:
        """
        Mencari manga berdasarkan genre yang diberikan.

        Args:
            genre (str): Genre manga yang dicari.
            page (int | None): Halaman pencarian (default None).

        Returns:
            SearchResult | None: Hasil pencarian manga berdasarkan genre atau None jika tidak ada hasil.
        """
        url = f"{BASE_URL}/genre/{genre}/page/{page}/" if page else f"{BASE_URL}/genre/{genre}/"
        x = cls(url)
        return x.get_search()
